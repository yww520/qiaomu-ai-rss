...
export const stateSchema = z.object({
  settings: z.object({
    ...
    weMpServerUrl: z.string().default('http://127.0.0.1:8001'),
    ...
    weMpServerUrl: 'http://127.0.0.1:8001', weMpToken: '',
...
export function safeUrl(value: string, base?: string): string | null {
  try {
    ...
    if (cleanVal.includes('mp.weixin.qq.com/s/')) {
      cleanVal = cleanVal.replace(/mp\.weixin\.qq\.com\/s\/([^?#]+)/, (_, token) => 'mp.weixin.qq.com/s/' + token.replace(/~/g, '_'));
    }
