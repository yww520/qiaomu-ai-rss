...
describe('WeMpClient', () => {
  it('formats base URL properly without trailing slash', () => {
    const client = new WeMpClient(() => 'http://127.0.0.1:8001/', () => '');
    expect(client.baseUrl).toBe('http://127.0.0.1:8001');
  });

  it('handles search and account transformation', async () => {
    const client = new WeMpClient(() => 'http://127.0.0.1:8001', () => 'test-token');
